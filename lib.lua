...
function Library:CreateWindow(...)
    ...
    if typeof(Config.Size) ~= 'UDim2' then Config.Size = UDim2.fromOffset(520, 700) end
    ...
    function Window:AddTab(Name)
        ...
        local TabButton = Library:Create('Frame', {
            BackgroundColor3 = Library.BackgroundColor;
            BorderColor3 = Library.OutlineColor;
            Size = UDim2.new(0, TabButtonWidth + 10, 1, 0);
            ZIndex = 1;
            Parent = TabArea;
        });
        ...
        local LeftSide = Library:Create('ScrollingFrame', {
            ...
            Size = UDim2.new(0.5, -12 + 2, 0, 597 + 2);
            CanvasSize = UDim2.new(0, 0, 0, 0);
            BottomImage = '';
            TopImage = '';
    ...
    Library:GiveSignal(InputService.InputBegan:Connect(function(inp, processed)
        ...
        if inp.KeyCode == Enum.KeyCode.Down then
            if #suggestions > 0 then
                ...
                Input.Text = cmdHistory[histIdx] or ""
                Input.CursorPosition = #Input.Text + 1
            end
            return
        end
...
-- ╔══════════════════════════════════════════════════════════════╗
-- ║              XWARE PERFORMANCE PROFILER v1.0                 ║
-- ║     Measure, Analyze, and Optimize Your Script's Speed       ║
-- ╚══════════════════════════════════════════════════════════════╝

function Library:CreateProfiler(MainWindow)
    local RunService = game:GetService("RunService")
    local TweenService = game:GetService("TweenService")
    local HttpService = game:GetService("HttpService")
    ...
    -- Add Debug Tab to window
    local ProfilerTab = MainWindow:AddTab("PROFILER")
    
    -- ============================================================
    -- LEFT SIDE - MEASUREMENT CONTROLS
    -- ============================================================
    local ControlGroup = ProfilerTab:AddLeftGroupbox("Profiler Controls")
    
    -- Recording status
    local RecordingStatus = ControlGroup:AddLabel("Status: ● IDLE", false)
    RecordingStatus.TextLabel.TextColor3 = Color3.fromRGB(150, 150, 150)
    
    -- Record button
    local RecordBtn = ControlGroup:AddButton("Start Recording", function()
        ProfilerData.recording = not ProfilerData.recording
        if ProfilerData.recording then
            ProfilerData.startTime = tick()
            ProfilerData.measurements = {}
            ProfilerData.lagSpikes = {}
            ProfilerData.frameTimes = {}
            ProfilerData.maxFrameTime = 0
            ProfilerData.totalFrames = 0
            ProfilerData.droppedFrames = 0
            RecordBtn:SetText("Stop Recording")
            RecordingStatus:SetText("Status: ● RECORDING")
            RecordingStatus.TextLabel.TextColor3 = Color3.fromRGB(255, 80, 80)
        else
            RecordBtn:SetText("Start Recording")
            RecordingStatus:SetText("Status: ● IDLE")
            RecordingStatus.TextLabel.TextColor3 = Color3.fromRGB(150, 150, 150)
            ProfilerTab:RefreshMeasurements()
        end
    end)
    
    -- Measure Function button
    ControlGroup:AddButton("Measure Function", function()
        local input = Library:CreateInputDialog("Function to Measure", "Enter function name or paste code:")
        if input then
            local success, result = pcall(loadstring("return " .. input))
            if success and type(result) == "function" then
                Profiler:Measure(result, input)
            else
                Library:Notify("Invalid function!", 2)
            end
        end
    end)
    ...
    -- Threshold settings
    local LagThreshold = ControlGroup:AddSlider("lag_threshold", {
        Text = "Lag Spike Threshold (ms)",
        Default = 100,
        Min = 16,
        Max = 500,
        Rounding = 0,
        Suffix = "ms",
        Callback = function(val)
            ProfilerData.lagThreshold = val
        end
    })
    ProfilerData.lagThreshold = 100
    
    -- ============================================================
    -- RIGHT SIDE - PERFORMANCE METRICS
    -- ============================================================
    local MetricsGroup = ProfilerTab:AddRightGroupbox("Real-time Metrics")
    
    -- FPS Monitor
    local CurrentFPS = MetricsGroup:AddVariableLabel("Current FPS: ", function()
        return ProfilerData.currentFPS or 0
    end, 0.2)
    
    local AvgFPS = MetricsGroup:AddVariableLabel("Average FPS: ", function()
        if ProfilerData.totalFrames == 0 then return 0 end
        return math.floor(ProfilerData.totalFrames / (tick() - ProfilerData.startTime))
    end, 1)
    
    local MinFPS = MetricsGroup:AddVariableLabel("Min FPS: ", function()
        return ProfilerData.minFPS or 0
    end, 1)
    
    MetricsGroup:AddDivider()
    
    -- Frame time monitor
    local CurrentFrameTime = MetricsGroup:AddVariableLabel("Frame Time: ", function()
        return string.format("%.2f ms", (ProfilerData.lastFrameTime or 0) * 1000)
    end, 0.2)
    
    local MaxFrameTime = MetricsGroup:AddVariableLabel("Max Frame Time: ", function()
        return string.format("%.2f ms", ProfilerData.maxFrameTime * 1000)
    end, 1)
    
    local DroppedFrames = MetricsGroup:AddVariableLabel("Dropped Frames: ", function()
        return ProfilerData.droppedFrames
    end, 1)
    
    -- ============================================================
    -- MEASUREMENT RESULTS
    -- ============================================================
    local ResultsGroup = ProfilerTab:AddLeftGroupbox("Function Measurements")
    
    local ResultsScroll = Library:Create('ScrollingFrame', {
        BackgroundColor3 = Library.BackgroundColor;
        BackgroundTransparency = 0;
        BorderSizePixel = 0;
        Size = UDim2.new(1, -4, 0, 200);
        CanvasSize = UDim2.new(0, 0, 0, 0);
        ScrollBarThickness = 3;
        ScrollBarImageColor3 = Library.AccentColor;
        ZIndex = 5;
        Parent = ResultsGroup.Container;
    })
    ...
    local MeasurementEntries = {}
    
    function ProfilerTab:RefreshMeasurements()
        -- Clear existing
        for _, entry in pairs(MeasurementEntries) do
            entry:Destroy()
        end
        MeasurementEntries = {}
        
        -- Sort measurements by total time
        local sorted = {}
        for name, data in pairs(ProfilerData.measurements) do
        ...
        for _, item in ipairs(sorted) do
            ...
            local StatsLabel = Library:CreateLabel({
                ...
                TextSize = 9;
                Text = string.format("Calls: %d | Total: %.2fms | Avg: %.3fms | Min: %.3fms | Max: %.3fms",
                    data.calls, data.totalTime * 1000, data.avgTime * 1000, data.minTime * 1000, data.maxTime * 1000);
            ...
            local maxTotal = sorted[1] and sorted[1].data.totalTime or 1
            local percent = data.totalTime / maxTotal
            
            local BarFill = Library:Create('Frame', {
                BackgroundColor3 = percent > 0.5 and Color3.fromRGB(255, 80, 80) or 
                                 percent > 0.2 and Color3.fromRGB(255, 200, 80) or 
            ...
            table.insert(MeasurementEntries, EntryFrame)
        end
    ...
    -- LAG SPIKE LOG
    -- ============================================================
    local LagGroup = ProfilerTab:AddRightGroupbox("Lag Spike Log")
    
    local LagScroll = Library:Create('ScrollingFrame', {
        BackgroundColor3 = Library.BackgroundColor;
        BackgroundTransparency = 0;
        BorderSizePixel = 0;
        ...
        ScrollBarImageColor3 = Library.AccentColor;
        ZIndex = 5;
        Parent = LagGroup.Container;
    })
    
    local LagLayout = Library:Create('UIListLayout', {
        Padding = UDim.new(0, 2);
        SortOrder = Enum.SortOrder.LayoutOrder;
        Parent = LagScroll;
    })
    ...
    function ProfilerTab:AddLagSpike(frameTime, timestamp)
        local EntryFrame = Library:Create('Frame', {
            BackgroundColor3 = Library.MainColor;
            BorderColor3 = Color3.fromRGB(255, 80, 80);
            BorderMode = Enum.BorderMode.Inset;
        ...
        local TimeLabel = Library:CreateLabel({
            ...
            TextSize = 9;
            Text = string.format("[%s]", os.date("%H:%M:%S", timestamp));
        ...
        local ValueLabel = Library:CreateLabel({
            ...
            TextSize = 9;
            Text = string.format("%.0fms spike", frameTime * 1000);
            TextColor3 = Color3.fromRGB(255, 100, 100);
            TextXAlignment = Enum.TextXAlignment.Right;
            Parent = EntryFrame;
        })
        
        table.insert(LagEntries, EntryFrame)
        
        -- Keep only last 20 spikes
        if #LagEntries > 20 then
            local oldest = table.remove(LagEntries, 1)
            oldest:Destroy()
        end
        
        LagScroll.CanvasSize = UDim2.new(0, 0, 0, LagLayout.AbsoluteContentSize.Y)
    end
    
    -- Clear lag log button
    LagGroup:AddButton("Clear Lag Log", function()
        for _, entry in pairs(LagEntries) do
            entry:Destroy()
        end
        LagEntries = {}
        LagScroll.CanvasSize = UDim2.new(0, 0, 0, 0)
    end)
    
    -- ============================================================
    -- OPTIMIZATION SUGGESTIONS
    -- ============================================================
    local SuggestionGroup = ProfilerTab:AddLeftGroupbox("Optimization Suggestions")
    
    local SuggestionScroll = Library:Create('ScrollingFrame', {
        BackgroundColor3 = Library.BackgroundColor;
        BackgroundTransparency = 0;
        BorderSizePixel = 0;
        Size = UDim2.new(1, -4, 0, 120);
        ...
        ScrollBarThickness = 3;
        ZIndex = 5;
        Parent = SuggestionGroup.Container;
    })
    
    local SuggestionLayout = Library:Create('UIListLayout', {
        Padding = UDim.new(0, 2);
        SortOrder = Enum.SortOrder.LayoutOrder;
        Parent = SuggestionScroll;
    })
    
    function ProfilerTab:UpdateSuggestions()
        -- Clear old
        ...
        -- Analyze measurements for bottlenecks
        for name, data in pairs(ProfilerData.measurements) do
            if data.totalTime > 0.1 then -- More than 100ms total
                table.insert(suggestions, string.format("⚠️ %s took %.0fms total (%d calls)", 
                    name, data.totalTime * 1000, data.calls))
            end
            if data.avgTime > 0.016 then -- More than 16ms average (bad for 60fps)
                table.insert(suggestions, string.format("🔴 %s averages %.1fms (will cause lag)", 
                    name, data.avgTime * 1000))
            end
        end
        
        -- Check frame rate
        if ProfilerData.droppedFrames > 10 then
            table.insert(suggestions, string.format("⚠️ %d dropped frames detected! Consider optimizing heavy functions", 
                ProfilerData.droppedFrames))
        end
        
        if ProfilerData.maxFrameTime > 0.1 then
            table.insert(suggestions, string.format("🔴 Severe lag spike: %.0fms! Check what happened at that moment", 
                ProfilerData.maxFrameTime * 1000))
        end
        
        if #suggestions == 0 then
            local NoSugg = Library:CreateLabel({
                Size = UDim2.new(1, -4, 0, 30);
                TextSize = 10;
                Text = "✓ No issues detected! Performance looks good.";
            ...
            for _, sugg in ipairs(suggestions) do
                local SuggLabel = Library:CreateLabel({
                    Size = UDim2.new(1, -4, 0, 16);
                    TextSize = 9;
                    Text = sugg;
        ...
        SuggestionScroll.CanvasSize = UDim2.new(0, 0, 0, SuggestionLayout.AbsoluteContentSize.Y)
    end
    
    -- ============================================================
    -- PROFILER CORE FUNCTIONS
    -- ============================================================
    
    local Profiler = {}
    
    -- Measure a specific function
    function Profiler:Measure(func, name, iterations)
        ...
        for i = 1, iterations do
            ...
            func()
            local iterEnd = tick()
            table.insert(times, iterEnd - iterStart)
        end
        ...
        -- Calculate stats
        local minTime = math.min(unpack(times))
        local maxTime = math.max(unpack(times))
        local sumTime = 0
        for _, t in ipairs(times) do sumTime = sumTime + t end
        local avgTime = sumTime / iterations
        
        -- Store measurement
        if not ProfilerData.measurements[name] then
            ProfilerData.measurements[name] = {
                calls = 0,
                totalTime = 0,
                minTime = math.huge,
                maxTime = 0,
        ...
        local data = ProfilerData.measurements[name]
        data.calls = data.calls + iterations
        data.totalTime = data.totalTime + totalTime
        data.minTime = math.min(data.minTime, minTime)
        data.maxTime = math.max(data.maxTime, maxTime)
        data.avgTime = data.totalTime / data.calls
        
        ProfilerTab:RefreshMeasurements()
        ProfilerTab:UpdateSuggestions()
        
        Library:Notify(string.format("Measured '%s': %.3fms avg over %d runs", 
            name, avgTime * 1000, iterations), 3)
    ...
    -- Profile a running function (for loops/updates)
    function Profiler:StartProfile(name)
        if ProfilerData.activeProfiles[name] then
            return false
        end
        
        ProfilerData.activeProfiles[name] = {
            startTime = tick(),
            calls = 0,
        }
        return true
    ...
    function Profiler:EndProfile(name)
        local profile = ProfilerData.activeProfiles[name]
        if not profile then return end
        
        local duration = tick() - profile.startTime
        ...
        ProfilerData.activeProfiles[name] = nil
        ProfilerTab:RefreshMeasurements()
        ProfilerTab:UpdateSuggestions()
    end
    
    -- Profile with automatic timing (decorator pattern)
    ...
    function Profiler:GetStats()
        return {
            measurements = ProfilerData.measurements,
            lagSpikes = ProfilerData.lagSpikes,
            frameStats = {
                currentFPS = ProfilerData.currentFPS,
                avgFPS = ProfilerData.totalFrames / (tick() - ProfilerData.startTime),
                maxFrameTime = ProfilerData.maxFrameTime,
                droppedFrames = ProfilerData.droppedFrames,
            }
        }
    end
    ...
    function Profiler:ExportData()
        local export = {
            ...
            measurements = ProfilerData.measurements,
            lagSpikes = ProfilerData.lagSpikes,
            frameStats = Profiler:GetStats().frameStats,
        }
        
        local json = HttpService:JSONEncode(export)
        
        -- Save to file
        local fileName = "profiler_export_" .. os.date("%Y%m%d_%H%M%S") .. ".json"
        writefile(fileName, json)
        
        Library:Notify(string.format("Exported to %s", fileName), 3)
        return json
    end
    
    -- ============================================================
    -- AUTO-MONITOR (Frame time tracking)
    -- ============================================================
    ...
    Library:GiveSignal(RunService.RenderStepped:Connect(function(deltaTime)
        if not ProfilerData.recording then return end
        
        local now = tick()
        local frameTime = now - lastTimestamp
        lastTimestamp = now
        
        -- Store frame time
        table.insert(ProfilerData.frameTimes, frameTime)
        if #ProfilerData.frameTimes > 300 then
            table.remove(ProfilerData.frameTimes, 1)
        end
        
        -- Update FPS
        ProfilerData.currentFPS = math.floor(1 / frameTime)
        if not ProfilerData.minFPS or ProfilerData.currentFPS < ProfilerData.minFPS then
            ProfilerData.minFPS = ProfilerData.currentFPS
        end
        
        ProfilerData.totalFrames = ProfilerData.totalFrames + 1
        ProfilerData.lastFrameTime = frameTime
        
        -- Track max frame time
        if frameTime > ProfilerData.maxFrameTime then
            ProfilerData.maxFrameTime = frameTime
        end
        
        -- Detect lag spikes
        if frameTime > (ProfilerData.lagThreshold or 100) / 1000 then
            ProfilerData.droppedFrames = ProfilerData.droppedFrames + 1
            table.insert(ProfilerData.lagSpikes, {
                time = frameTime,
                timestamp = now,
            })
            ProfilerTab:AddLagSpike(frameTime, now)
            ProfilerTab:UpdateSuggestions()
        end
    end))
    
    -- Export button
    local ExportBtn = ControlGroup:AddButton("Export Profiler Data", function()
        Profiler:ExportData()
    end)
    
    -- Clear measurements button
    ControlGroup:AddButton("Clear Measurements", function()
        ProfilerData.measurements = {}
        ProfilerTab:RefreshMeasurements()
        ProfilerTab:UpdateSuggestions()
        Library:Notify("All measurements cleared!", 2)
    end)
    
    -- ============================================================
    -- EXAMPLE USAGE BUTTONS
    -- ============================================================
    local ExamplesGroup = ProfilerTab:AddRightGroupbox("Test Examples")
    
    ExamplesGroup:AddButton("Test: Heavy Loop", function()
        Profiler:Measure(function()
            local sum = 0
            for i = 1, 100000 do
                sum = sum + i
            end
        end, "Heavy Loop (100k iterations)")
    end)
    
    ExamplesGroup:AddButton("Test: String Concat", function()
        Profiler:Measure(function()
            local str = ""
            for i = 1, 1000 do
                str = str .. tostring(i)
    ...
    ExamplesGroup:AddButton("Test: Table Operations", function()
        Profiler:Measure(function()
            local t = {}
            for i = 1, 10000 do
                t[i] = i
...
function Library:CreateInputDialog(title, prompt)
    ...
    frame.BackgroundColor3 = self.MainColor
    frame.BorderColor3 = self.AccentColor
    ...
    title.Text = title
    title.TextColor3 = self.FontColor
    ...
    input.BackgroundColor3 = self.BackgroundColor
    input.BorderColor3 = self.OutlineColor
    input.BorderMode = Enum.BorderMode.Inset
    input.TextColor3 = self.FontColor
    input.FontFace = self.Font
    input.TextSize = 12
    ...
    confirm.Position = UDim2.new(1, -90, 1, -35)
    confirm.Text = "OK"
    confirm.BackgroundColor3 = self.AccentColor
    confirm.TextColor3 = Color3.new(1, 1, 1)
    confirm.FontFace = self.Font
    confirm.TextSize = 12
